...
export default class StyleSelectBoxEditor extends PureComponent {
	...
	componentDidUpdate(prevProps) {
		...
		if (
			JSON.stringify(getDataLoaderOptionsForProps(this.props)) !==
			JSON.stringify(getDataLoaderOptionsForProps(prevProps))
		) {
			this.loadSelectBoxOptions();
		}
	}
	...
	loadSelectBoxOptions() {
		if (this.props.options.dataSourceIdentifier) {
			...
			this.props.dataSourcesDataLoader
				.resolveValue(
					getDataLoaderOptionsForProps(this.props),
					this.props.value
				)
	...
	render() {
		...
		const processedSelectBoxOptions = processSelectBoxOptions(
			i18nRegistry,
		...
		return (
			<SelectBox
				...
				displaySearchBox={shouldDisplaySearchBox(
					options,
					processedSelectBoxOptions
				)}
